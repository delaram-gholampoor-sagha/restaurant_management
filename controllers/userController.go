...
import (
	...
	helper "github.com/Delaram-Gholampoor-Sagha/restaurant_management/helpers"
...
func GetUser() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "erro ocured while listing user items"})
		}
...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// converting the json data from postman to something that golang undestands
...
func LogIn() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// convert the logIn data from postman which is in json to golang readable fromat
		if err := c.BindJSON(&user); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}

		// find a user with that email and see if that use even exists
